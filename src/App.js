...
const audio = new Audio("/ForMyCrush/img/bg.mp3");
...
function App() {
  ...
  if (breakHeart) {
    return (<div className="min-h-screen w-screen relative flex flex-wrap overflow-hidden items-center justify-center bg-black">
      <img src={"/ForMyCrush/img/t10.png"} alt=""
        className="h-40" />
      <motion.p
        initial={{ width: 0 }}
        animate={{ width: 250, }}
...
const Loading = ({ onDone }) => {
  ...
  const loadImage = (filename) => new Promise((r) => {
    let link = "/ForMyCrush/img/" + filename;
    let img = new Image();
    img.onload = () => {
      r(link);
    }
      ...
      <motion.div className="">
        ...
        <div className="flex flex-col items-center relative mb-16">
          <div className="h-48 relative  hover:scale-[4] transition-transform z-20">
            <motion.img src={"/ForMyCrush/img/dang-nhi.jpg"} alt=""
            ...
            />
            <motion.img src={"/ForMyCrush/img/point.png"} alt=""
...
const DangNhi = ({ onDone, isMormal, notAccept, }) => {
  ...
  const timeline = [
    [3, isMormal ? "Helloooo, anh chỉ hỏi vậy thui." : "Anh biết ngay là em mà!!!", null, false,],
    [2, isMormal ? "\"Mẫn Nhi\"" : "My \"công chúa\"", null, false],
    ...
    [2, "Mừng ghêêêêêêêêêêêêêêêêêêêê!", null, false],
    [2, "Mừng ghêêêêêêêêêêêêêêêêêêêê x2!", "t3", false],
    ...
    [2, "Thôi nhé", null, 3],
    [2, "Thank for reading! Like you 😘🥰...\n\nQ & A to me", null, 4, "text-[5rem] text-black font-sans leading-[1.5]"],
  ]
    ...
    >
      <div className="relative text-center flex flex-col items-center">
        <div className={"relative text-4xl text-center font-black font-main transition-all duration-300 " + (mode ? "a-up" : "a-down")} >
          ...
          {timeline[index][2] && <>
            <img src={"/ForMyCrush/img/" + timeline[index][2] + ".png"} alt=""
